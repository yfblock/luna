...
```text
先运行原 root LKL tty/shell 回归并正常 halt/cleanup
  → manager 使用仅 root ELF 映射的页对齐 probe
  → 创建 fault-mode child
...
manager 现在为每个 `luna-lkl-task` 预配置 64 个真实 host-thread slot：
...
其中 63 个用于 LKL host thread，最后 1 个专供 polling oneshot timer。manager 还预建 128 个
同步 Notification，供 sem/mutex 使用。child 不持有 VKA、Untyped 或 manager CSpace。
...
child 使用 `mem=8M` 时会在完整文件系统初始化阶段 OOM，因此与稳定 root 基线对齐为 `mem=16M`
和 32MiB host heap。child ELF 扩大后，原 manager probe 地址一度落入 child BSS；现在 probe 位于
仅 root 链接的尾部对象中，运行时地址高于 child ELF 映射末端，VMFault 断言重新有效。

加载几十 MiB child ELF 需要为数千个 frame/mapping 保存 allocman metadata。root 静态 metadata
pool 从 1MiB 扩到 4MiB；迁移期 QEMU 默认内存为 1GiB，可用 `LUNA_QEMU_MEM` 覆盖。

## 当前自动验证顺序

迁移期 smoke 先运行稳定 root tty/shell；收到 `exit` 并完成 root `lkl_sys_halt()`/cleanup 后，再执行：

```text
fault-mode child boot → timer → halt → manager-private VMFault
  → 销毁 child 和全部固定资源
clean-mode child boot → timer → halt → DONE
...
Phase 2.1b 已完成 child kernel boot、timer 和 halt。下一切片迁移 tty/shell：

1. manager 保留 COM1 I/O port，通过受控输入代理或共享 SPSC ring 向 child 提供字符，不直接授予
   child manager 的完整 I/O capability；
2. child 实现 `console_start/console_take`，创建 `/dev/ttyLKL0` 并运行现有 shell；
3. 将当前 shell smoke 改为驱动 child，保持 timer、文件操作和正常关机断言；
4. child shell 等价后删除 root-hosted LKL、root 32MiB heap 和重复 host ops。
