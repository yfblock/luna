...
独立 VSpace 的 LKL 中执行。root LKL 仅保留一次非交互 boot/halt 对照，不再消费控制台输入。

## 当前自动验证顺序

迁移期 smoke 先自动运行一次非交互 root LKL boot/halt，再执行：
...
Phase 2.1c 已完成 child tty/shell。下一切片删除 root-hosted LKL：

1. manager 独立完成 PIT/TSC 校准并把频率交给 child；
2. root target 不再链接 `lkl.o`、root 32MiB heap、旧 host ops 或 shell；
3. 使用动态映射的 manager-private probe 取代当前依赖 root ELF 尾部布局的 probe；
4. 保持 child shell、fault/restart 和 `LUNA_SHUTDOWN_OK` smoke 全部通过。
