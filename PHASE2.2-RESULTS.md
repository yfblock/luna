# Phase 2.2 — 可回收页映射、同步/TLS 语义与生命周期压力
...
后续切片补齐了可计数 semaphore、mutex owner 检查和 TLS destructor 多轮重入语义；这些测试同样
在每个 fault/stress/clean child 的 `lkl_init()` 前执行。
...
下一轮再次执行。第 4 轮后仍存在的 value 会被清除，避免固定 tid 槽复用时泄漏到下一线程。
...
Phase 2.2 剩余项是更细的 thread join 与 timer cancel/rearm 并发压力测试。
