...
下一次迭代进入 Phase 2.3 的 virtio-block 最小闭环：先确定 backend/transport 边界，再完成单块设备
识别、读写和持久化 smoke；继续保留 `tools/smoke-test.sh` 的 100 轮生命周期压力作为所有结构调整的
回归门槛。默认 QEMU 内存为 512MiB。
