...
Phase 2.1 下一切片：删除 root-hosted LKL 副本和重复 host ops，manager 独立校准 TSC，并把隔离
probe 改为动态私有映射。完成后 root 只保留资源分配、fault 诊断和生命周期管理。
...
下一次迭代从移除 root-hosted LKL 副本开始，并保留当前 `tools/smoke-test.sh` 作为所有结构调整的
回归门槛。
