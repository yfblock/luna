...
- [x] 对 child 创建的 process、heap、disk、network、资源池和控制 cap 七阶段注入回滚。
- [x] 修复未触碰的大范围 heap reservation 在 sel4utils teardown 中的中层 RESERVED 清理问题。
- [x] 增加 socket 性能硬下限、reviewed reference 和资源/capability 等值门槛。
- [x] 增加多 QEMU 共享 ext4、逐轮 FSCK、性能汇总和资源漂移检测。
- [x] 增加 QEMU/helper/TAP 后端能力探测和失败 marker 快速终止。
- [x] 实测 socket、slirp、passt 和 TAP；passt 对不支持的 gateway ICMP 显式降级，但 TCP/UDP 仍为硬门槛。

完成状态：Phase 2.7 已完成。实现、宿主准备条件和验证结果见 `PHASE2.7-RESULTS.md`。

## Phase 2.8：稳态性能与通知化

- [x] Block 使用一次完整 warm-up、7 个 cache-normalized 正式样本和 p50/p95/p99。
- [x] 输出 queue/worker/IPC/copy/flush/completion 分段，定位旧双峰的首次调度、cache 与 journal 状态污染。
- [x] fault/clean child 保留 8192 页 full allocator audit，100 个 stress child 使用 36 页 light profile。
- [x] 用现有同步池内的 LKL semaphore 替换 Block queue、batch completion 和 async flush 忙等待。
- [x] 扩展 virtio ethdriver，实现 descriptor 层 16-packet INTx budget 和 poll-worker coalescing。
- [x] BusyBox copyfd 使用 64KiB buffer，pipeline 使用 256KiB LKL pipe，并硬性检查平均 I/O 尺寸。
- [x] 保留 100-child、跨 QEMU ext4/FSCK、socket/slirp/passt/TAP、网络压力和 `forbidden=0`。

完成状态：Phase 2.8 已完成。实现和正式 benchmark 见 `PHASE2.8-RESULTS.md`。

## Phase 2.9：网络热路径通知化与自适应批处理

- [x] 正式 TX 扩展到 37.5 MiB，RX 持续至少 3 秒，并记录 cycles/packet、batch、wait 和 yield。
- [x] child TX queue 使用 counting semaphore，completion 释放 producer token，`hot_yields=0`。
- [x] 固定 5 ms coalescing 改为自适应 50/100/200 μs，满 16 包立即提交。
- [x] poll worker 成为 virtio-net driver 唯一调用者，固定 IRQ relay 只转发 Notification 和 ack。
- [x] RX 使用 produced/consumed/waiting 握手，消除 batch drain 的丢失唤醒。
- [x] timer 持续 TSC polling 改为 manager PIT one-shot Notification，并合并较晚 deadline。
- [x] pending TX response 使用 5 ms manager one-shot、bounded poll 和单次 timeout retry 收敛。
- [x] 保留 manager PCI/DMA 独占、100-child、跨 QEMU persistence/FSCK、`fallback_polls=0` 和资源等值。

完成状态：Phase 2.9 已完成。socket、slirp、passt 与后端能力边界、性能取舍和正式结果见
`PHASE2.9-RESULTS.md`。当前宿主没有可配置 TAP 的 `CAP_NET_ADMIN`，能力探测会给出明确原因。

## Phase 2.10：共享 descriptor ring 与 reduced-copy

下一阶段应先减少 child/manager 共享窗口 copy，再考虑 virtio 多队列：

- TX 从 child packet queue → shared slots → manager queue → DMA 的最多 4 次复制降到 2 次；
- RX 从 DMA → manager queue → shared slots → LKL iovec 的 3 次复制降到 1 次；
- descriptor 所有权使用 generation/index 和 acquire/release 明确表达，不重新引入 driver 多 owner；
- 保留 Phase 2.9 counting semaphore、RX waitqueue、INTx budget、100-child 和四后端回归；
- 先给出 copy 次数、cycles/packet、cache footprint 和尾延迟，再决定是否增加多队列。
...
Phase 2.9 已完成；下一项是 Phase 2.10 reduced-copy，之后再评估多队列或 Phase 3 UINTR。也可继续
扩展静态 nofork applet。继续保留 `tools/smoke-test.sh`
