...
`lkl.o` 也已链接进 child 并报告 `LKL_LINKED`。Phase 2.1b 进一步建立 64 个固定 TCB slot、128 个
同步 Notification、32MiB child heap 和专用 polling timer TCB；child 已独立完成 `lkl_init()`、
`lkl_start_kernel("mem=16M")`、真实 oneshot timer 和 `lkl_sys_halt()`。fault child 销毁后 replacement
child 能再次完整 boot/halt，原 root tty/shell smoke 也继续通过。
实现与证据见 `PHASE2.1-RESULTS.md`。

Phase 2.1 下一切片：manager 代理 COM1 输入，child 实现 `console_start/console_take` 并运行现有
tty shell。shell smoke 切换到 child 后，删除 root-hosted LKL 副本，使 root 只保留资源分配、fault
诊断和生命周期管理。
...
下一次迭代从 Phase 2.1 的 tty/shell 输入代理开始，并保留当前 `tools/smoke-test.sh` 作为所有结构
调整的回归门槛。
