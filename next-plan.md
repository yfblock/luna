...
下一次迭代进入 Phase 2.4：先把最小静态用户程序放入持久 rootfs 并打通 syscall shim/exec，再扩展
clone/fork/wait 与 BusyBox；继续保留 `tools/smoke-test.sh` 的 100 轮生命周期和 ext4 重挂载压力作为
所有结构调整的回归门槛。默认 QEMU 内存为 512MiB。
