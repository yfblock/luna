...
Phase 2.2 剩余工作是可计数 semaphore、mutex owner 错误报告、TLS destructor 重入，以及 thread/timer
并发压力语义。
...
下一次迭代继续 Phase 2.2 的同步与 TLS 语义，并保留当前 `tools/smoke-test.sh` 的 100 轮生命周期
压力作为所有结构调整的回归门槛。默认 QEMU 内存为 512MiB。
