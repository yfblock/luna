...
下一次迭代进入 Phase 2.5 virtio-net：先完成单队列收发与 IRQ/kick 边界，再接 DHCP 或固定地址、
ICMP 和 TCP smoke；继续保留 `tools/smoke-test.sh` 的 100 轮生命周期、ext4 重挂载和 BusyBox
spawn/join 作为所有结构调整的回归门槛。默认 QEMU 内存为 512MiB。
