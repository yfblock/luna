...
`head`、`wc`、`cut`、`uniq`、`true`、`false`、`fsync`、`sync`。例如：
...
ivshmem、静态 runtime 最多 4 个 worker 且不支持通用 native fork/exec、当前网络仅支持单队列
legacy INTx，且默认测试对端是确定性的 localhost 二层 peer，不是互联网出口。
