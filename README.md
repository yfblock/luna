...
manager 私有 backing 的 virtio-block、ext4 持久根文件系统和跨 100 次 child 重建的数据校验。root
Phase 2.4 又加入静态 host-program ABI，并从持久 rootfs 启动最小 BusyBox `ash`/`cat`，通过专用
program thread 执行、退出和 join。root manager 不再链接或运行 LKL。详见
[`PHASE2.1-RESULTS.md`](PHASE2.1-RESULTS.md) 和
[`PHASE2.2-RESULTS.md`](PHASE2.2-RESULTS.md)、
[`PHASE2.3-RESULTS.md`](PHASE2.3-RESULTS.md) 和
...
静态 ABI 尚不支持 pipeline/native fork/exec，以及尚无网络。
