...
诊断和销毁重建路径；child 现在也能独立启动/关闭 LKL 并运行真实 oneshot timer。tty/shell 仍由
root 基线路径承载，详见
...
> root task 明确挂起。Phase 2.1 迁移期间，root shell 退出后还会自动执行隔离 child 的两次
> boot/halt、fault 和重建验证，然后才打印 `LUNA_SHUTDOWN_OK`。交互模式下用 Ctrl-A X 退出 QEMU。
...
Phase 2.1b 已把完整启动所需的 thread、sem/mutex、TLS、memory、time 和 oneshot timer host ops
迁入 child。manager 为每个 child 预建 64 个 TCB slot（63 个 LKL 线程 + 1 个 timer service）和
128 个同步 Notification；child 使用 32MiB host heap，以 `mem=16M` 完成 `lkl_start_kernel()` 与
`lkl_sys_halt()`。fault child 销毁后，replacement child 会再次完整 boot/halt，证明固定池和 ELF
映射能够回收。当前 smoke 先跑稳定 root tty/shell，`exit` 后再跑 child 隔离验证，避免在迁移期
反向重启 root-hosted LKL。

当前限制仍包括：child 尚无 tty 输入和 shell、bump heap 不回收、mutex/sem 为二值、无 virtio
块设备/网络。下一切片将把 COM1 输入代理、LKL tty 和 shell 迁入 child，随后删除 root-hosted
LKL 副本。后续工作见 [`next-plan.md`](next-plan.md)。
