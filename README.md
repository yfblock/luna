# luna — LKL on seL4 (Phase 1.1 + 交互 shell) ✅ 已跑通
...
`lkl_sys_*` 跑在 LKL 内核上，`free` 读到真实 `/proc/meminfo`）。证据见 `EVIDENCE_lkl_on_sel4_boot.txt`、
`EVIDENCE_shell.txt`、`EVIDENCE_serial_input.txt`，详见 [`PHASE1-RESULTS.md`](PHASE1-RESULTS.md)。
...
```
luna/
├── PHASE1-RESULTS.md            # 实际实现 + 验证结果 + 踩坑（必读）
...
| timer | stub（pc99 ltimer IRQ 冲突；boot 路径无需 tick） |
| console | `seL4_DebugPutChar` |

## 复现

首次克隆请先按 [`DEPENDENCIES.md`](DEPENDENCIES.md) 获取固定版本的 seL4/LKL 源码并应用
本仓跟踪的 LKL tty 补丁。随后见 [`PHASE1-RESULTS.md`](PHASE1-RESULTS.md) “复现”一节。要点：
```sh
make -C deps/lkl-linux/tools/lkl -j                       # 建 liblkl.a
ar x deps/lkl-linux/tools/lkl/liblkl.a lkl.o              # 抽取干净内核对象
export LUNA_SETTINGS=$PWD/deps/lkl_settings.cmake
export LKL_LINUX_DIR=$PWD/deps/lkl-linux
export LKL_KERNEL_OBJ=$PWD/build-artifacts/lkl-kernel.o
...
```
环境补丁：`~/bin/xmllint` exit-0 stub；`pip3 install --user --break-system-packages sh aenum pyelftools sortedcontainers future jinja2`。
...
`mount <src> <dir> <fstype>`、`free`（读 `/proc/meminfo`）、`help`、`exit`。

> 注：`run.sh` 会 `make -C deps/lkl-linux/tools/lkl` 重建 LKL（含 lkl_tty 驱动）并重新抽取 lkl.o。
> halt 路径在 LKL 协作模型下会触发 reboot 后的次生 fault（shell 已 exit，可忽略）。

完整 busybox sh 仍需任务 9/11（静态 lkl syscall shim + clone/fork），但**控制台 I/O 已经由 LKL tty 解决**。

## 状态

Phase 1.1 **完成并验证**：LKL 作为 seL4 纯用户态 Task 完成内核初始化并以 init 身份打印 hello。
已知限制（timer stub、halt 次生 fault、bump 堆不回收、无 virtio 数据通路）见 PHASE1-RESULTS。
后续：真实 timer 驱动、独立子任务沙箱化（DESIGN §5）、UINTR（下一阶段）。
