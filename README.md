...
Phase 2.7 又加入 child 创建七阶段回滚审计、多 QEMU 稳定性/FSCK 驱动、性能硬门槛与 reference
对比、后端能力探测和失败 marker 快速终止。
Phase 2.8 又将 Block 改为 warm-up + 7 个稳态样本并定位旧双峰的首次调度/cache/journal 状态污染，
把 100 个 stress child 的 allocator 改为 36 页 light profile，以 semaphore 替换 Block 背压/完成/
flush 忙等待，在 virtio descriptor 层实现 16-packet INTx budget/coalescing，并把 BusyBox pipeline
批量提升到 64 KiB I/O 与 256 KiB pipe。
Phase 2.9 又把 37.5 MiB TX 与 3 秒持续 RX 纳入正式门槛，以 counting semaphore 替换 TX queue
热路径背压，将 5 ms 固定 coalescing 改为自适应 50–200 μs，并让 poll worker 成为 virtio-net
driver 的唯一调用者。IRQ relay 只转发 Notification/ack，RX 使用 produced/consumed/waiting
握手避免丢失唤醒，timer 则改为 PIT one-shot Notification 并合并较晚 deadline；pending TX
response 使用 5 ms bounded recheck 和单次 timeout retry，避免无后续 INTx 时永久停滞。
root manager 不再链接或运行 LKL。详见
[`PHASE2.9-RESULTS.md`](PHASE2.9-RESULTS.md)、
[`PHASE2.8-RESULTS.md`](PHASE2.8-RESULTS.md)、
[`PERFORMANCE-RESULTS.md`](PERFORMANCE-RESULTS.md)、
[`PHASE2.7-RESULTS.md`](PHASE2.7-RESULTS.md)、
...
├── PHASE2.7-RESULTS.md          # 回滚审计、稳定性门槛与多后端实测
├── PHASE2.8-RESULTS.md          # Block 稳态、allocator 分级、INTx budget 与 pipeline 批量化
├── PHASE2.9-RESULTS.md          # 网络热路径通知化、自适应批处理与 one-shot timer
...
│       ├── isolation_timer.c    # manager PIT one-shot IRQ 与 child wake Notification
...
| block | LKL virtio-mmio/blk + 64KiB IPC 窗 + 8-batch semaphore queue；manager 独占 16MiB backing |
| static program | BusyBox ash + 4 个隔离 worker + 64KiB copy I/O + 256KiB LKL pipe + 1MiB bounded arena |
| network | manager 16-packet INTx budget/coalescing + polling fallback + bounded TX/RX queue + 68KiB 窗 |
...
`run.sh` 默认给 QEMU 配置 512MiB 内存，root allocman metadata pool 为 12MiB，以装载 child ELF、
...
LUNA_QEMU=/usr/bin/qemu-system-x86_64 ./run.sh --net-backend slirp
./tools/network-backends.py
```

TAP 接口需由宿主预先创建并配置 `10.0.2.2/24`，并关闭该专用接口的 IPv6 与 multicast，避免
legacy INTx 中断风暴：

```sh
sudo ip tuntap add dev luna-tap0 mode tap user "$USER"
sudo ip addr add 10.0.2.2/24 dev luna-tap0
sudo ip link set luna-tap0 up
sudo sysctl -w net.ipv6.conf.luna-tap0.disable_ipv6=1
sudo ip link set dev luna-tap0 multicast off
```

slirp、passt 和 TAP 使用
`tools/net-service.py` 提供与默认回归等价的 TCP echo、RX burst 和 TX 完整性确认。
老版本 passt 不一定对映射 gateway 提供 ICMP echo；此时回归要求明确的
`LUNA_NETWORK_ICMP_UNAVAILABLE`，但仍强制通过 TCP、双向 UDP 和完整性压力测试。socket、slirp
和 TAP 仍强制要求 ICMP 成功。
...
child。当前 manager 为每个 child 预建 50 个 TCB（47 个 LKL slot、timer、console 和初始 TCB）与
146 个 Notification（49 个 join、96 个同步 slot 和 receive-only RX），并只授予 child
`0x3f8–0x3ff` 的 COM1 I/O-port capability。clean child 使用
...
Phase 2.7 已完成。manager 在真正启动 child 前对 process、heap reservation、disk/network window、
TCB/Notification pool 和控制 cap 七个阶段逐一注入回滚，随后仍通过正式 100 轮生命周期。稳定性工具
用同一 ext4 backing 运行多个独立 QEMU，每轮执行 FSCK、性能硬门槛和 capability/resource 等值检查；
socket、slirp、passt 和 TAP 的实际结果与宿主条件见 `PHASE2.7-RESULTS.md`。

Phase 2.8 已完成。Block benchmark 使用一次 warm-up 和 7 个 cache-normalized 正式样本，输出完整
p50/p95/p99 与 queue/worker/IPC/copy/flush 分段；100 个 stress child 使用 36 页 light allocator，
首尾 audit child 仍完整触碰 8192 页。Block 数据通路的 queue/completion/flush 已由现有同步池中的
semaphore 唤醒；virtio legacy INTx 在 descriptor 层限制为每次 16 个 packet 并由 poll worker 合并
余量；BusyBox pipeline 使用 64KiB copy buffer 和 256KiB pipe。完整证据见 `PHASE2.8-RESULTS.md`。
...
budgeted legacy INTx，数据仍经过 child queue、共享窗口、manager queue 和 DMA buffer 多次复制。
socket 默认后端是确定性的 localhost 二层 peer；slirp/passt/TAP 是否可访问外网取决于
